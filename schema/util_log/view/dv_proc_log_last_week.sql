CREATE OR REPLACE VIEW util_log.dv_proc_log_last_week
AS
SELECT row_number () OVER ( ORDER BY dpl.tmsp_exec DESC ) AS rn,
    ...
    WHERE dpl.tmsp_exec > now () - '7 day'::interval
    ORDER BY dpl.tmsp_exec DESC ;

COMMENT ON VIEW util_log.dv_proc_log_last_week IS 'View of database log entries for the past 7 days.' ;
